...
export async function setIndexDB() {
  ...
  return new Promise((resolve, reject) => {
    ...
    // 打开数据库
    const SCENE_DB = window.indexedDB.open('threeSceneEditor', 1)
    ...
    SCENE_DB.onsuccess = event => {
      ...
      function getRequest(url,name) {
        ...
        return new Promise((getResolve, getReject) => {
          ...
          getRequest.onsuccess = event => {
            ...
            if(event.target.result)  getResolve({ event, status: true, message: '获取成功',url: URL.createObjectURL(event.target.result.blob) })
      ...
      resolve({ getRequest, clearRequest, DATABASE, status: true, message: '数据库打开成功' })
